...
// --- [BADLAV 1] - `AnimatedStatProps` mein naya prop add kiya gaya ---
interface AnimatedStatProps {
  finalValue: string;
  label: string;
  duration?: number;
  iconColor?: string;
  showPlusSign?: boolean; // Naya prop, default roop se true hoga
}

// --- [BADLAV 2] - `AnimatedStat` component ko naya prop use karne ke liye update kiya gaya ---
const AnimatedStat: React.FC<AnimatedStatProps> = ({
  finalValue,
  label,
  duration = 2000,
  iconColor = "text-pharma-green",
  showPlusSign = true, // Default value set ki gayi
}) => {
  ...
  return (
    <div ref={ref} className="text-center px-2 md:px-4">
      ...
      {/* Plus sign ko conditionally render kiya gaya */}
      {showPlusSign && (
...
const ProductsSection = () => {
  ...
  const featuredProducts = [
    { name: "Fladecort 6", description: "A potent glucocorticoid used to treat a wide range of inflammatory and autoimmune conditions.", image: Faldecort, slug: "lexprate-cr", gradientClass: "from-red-500 to-pink-500", },
    { name: "Linzocast 600", description: "A last-resort antibiotic for treating serious infections caused by multi-drug resistant bacteria.", image: Linzocast, slug: "calvilux", gradientClass: "from-blue-500 to-cyan-500", },
    { name: "Livofolate", description: "A comprehensive formula with active B-vitamins and Omega-3 to support nerve, brain, and heart health.", image: Livofolate, slug: "multilux", gradientClass: "from-green-500 to-emerald-500", },
    { name: "Livozorb XT Syp", description: "A palatable iron supplement syrup, ideal for children and adults to effectively treat iron deficiency anemia.", image: Livozorbxtsup, slug: "neurolux", gradientClass: "from-purple-500 to-indigo-500", },
    { name: "Livozorb XT", description: "A high-potency iron formulation with Zinc to support against anemia and boost immune function.", image: Livozorbxt, slug: "cardiolux", gradientClass: "from-orange-500 to-red-500", },
    { name: "Luxbone Fem", description: "A specialized formula expertly crafted to meet the unique bone health needs of women, especially post-menopause.", image: LuxboneFem, slug: "diabetlux", gradientClass: "from-teal-500 to-blue-500", },
    { name: "Luxbone", description: "A premium, comprehensive formula for building and maintaining strong, healthy bones in adults.", image: Luxbone, slug: "immunolux", gradientClass: "from-yellow-500 to-orange-500", },
    { name: "Pandoride DSR", description: "A dual-action capsule for effective, sustained relief from acid reflux, heartburn, and indigestion.", image: Pandoride, slug: "gastrolux", gradientClass: "from-pink-500 to-rose-500", },
  ];
  ...
  return (
    <section
      ...
      style={{ backgroundImage: `linear-gradient(rgba(15, 23, 42, 0.96), rgba(23, 37, 84, 0.98)), url(${sectionBackgroundImage})`, backgroundSize: "cover", backgroundPosition: "center center", backgroundAttachment: "fixed", }}
    >
          ...
          <div className="grid grid-cols-2 md:grid-cols-[1fr_auto_1fr_auto_1fr_auto_1fr] gap-x-2 gap-y-8 md:gap-x-0 md:gap-y-0 items-center">
            {companyStats.map((stat, index) => (
              <React.Fragment key={stat.label}>
                {/* --- [BADLAV 4] - Naye prop ko `AnimatedStat` component mein pass kiya gaya --- */}
                <AnimatedStat
                  finalValue={stat.value}
                  label={stat.label}
                  iconColor={stat.iconColor}
                  showPlusSign={stat.showPlus} // Yahan naya prop pass kiya gaya
                />
              ...
              <motion.div className="embla__slide flex-[0_0_85%] sm:flex-[0_0_50%] md:flex-[0_0_33.33%] lg:flex-[0_0_25%] min-w-0 pl-3" key={product.slug} variants={cardVariant}>
                <div className="group relative bg-slate-800/70 backdrop-blur-md rounded-2xl overflow-hidden shadow-xl border border-slate-700/80 hover:border-pharma-blue/60 transition-all duration-300 transform hover:-translate-y-1.5 h-full flex flex-col">
                  ...
                  <div className="p-5 flex flex-col flex-grow">
                    ...
                    <Button variant="outline" size="sm" className="w-full mt-auto text-slate-200 border-slate-600 hover:text-white hover:bg-pharma-blue/80 hover:border-pharma-blue/80 transition-all duration-300" onClick={() => handleProductClick(product.slug)}>Learn More<ArrowRight className="ml-2 h-4 w-4 group-hover:translate-x-1 transition-transform" /></Button>
                  </div>
                </div>
              </motion.div>
            ))}
